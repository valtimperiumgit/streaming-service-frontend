import "../movie-player/MoviePlayer.css"
...
import {getTvEpisodeAsyncThunk, getTvShowAsyncThunk, tvShowsState} from "../../features/tv-shows/store/TvShowsSlice";

const TvShowPlayer = ( ) => {
    const dispatch: AppDispatch = useDispatch();
    let tvShow = useSelector(tvShowsState).tvShowForWatching;
    let episode = useSelector(tvShowsState).episodeForWatching;
    const [playing, setPlaying] = useState(true);
    const [volume, setVolume] = useState(0.8);
    const playerRef = useRef<ReactPlayer>(null);
    const [played, setPlayed] = useState<number>(0); // Для отслеживания прогресса видео
    const [duration, setDuration] = useState<number>(episode?.duration !== undefined ? episode?.duration * 60 : 200);
    ...
    useEffect(() => {
        dispatch(getTvShowAsyncThunk(location?.state.tvShowId));
        dispatch(getTvEpisodeAsyncThunk(location?.state.episodeId));
        ...
        return () => {
            localStorage.setItem(location?.state.episodeId, (playedRef.current * durationRef.current).toString())
        };
    }, [location.state]);
